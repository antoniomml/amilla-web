...
const translations = {
  en: {
    ...
    pauseTyping: "Pause the animated text",
    resumeTyping: "Resume the animated text",
    phrases: [
      "at Ryanair",
  ...
  es: {
    ...
    pauseTyping: "Pausar el texto animado",
    resumeTyping: "Reanudar el texto animado",
    phrases: [
      "en Ryanair",
  ...
  fr: {
    ...
    pauseTyping: "Mettre le texte animé en pause",
    resumeTyping: "Reprendre le texte animé",
    phrases: [
      "chez Ryanair",
...
if (typewriterText && phrases?.length) {
  ...
  const syncTypewriterControl = () => {
    ...
    if (reducedMotion.matches) {
      ...
      typewriter.removeAttribute("aria-label");
      typewriter.removeAttribute("aria-pressed");
      delete typewriter.dataset.paused;
      return;
    }

    typewriter.disabled = false;
    typewriter.setAttribute("aria-pressed", String(paused));
    typewriter.setAttribute(
      "aria-label",
      paused ? copy.resumeTyping : copy.pauseTyping,
    );
    typewriter.dataset.paused = paused ? "true" : "false";
  };

  const scheduleTick = (delay) => {
    stopTyping();

    if (!document.hidden && !reducedMotion.matches && !paused) {
  ...
  const tick = () => {
    ...
    typewriterText.textContent = currentPhrase.slice(0, Math.max(0, charIndex));
...
if (shapes.length && window.matchMedia("(any-pointer: fine)").matches) {
